...
(function (window, $) {
    ...
    function getTrans(page, trans, cb) {
        ...
        SG.Plugins.DBP.trans(trans, begin, end, function (j) {
            ...
            $.get("load_page_ayas.php?b_sura=" + b_sura + "&b_aya=" + b_aya + "&e_sura=" + e_sura + "&e_aya=" + e_aya + "&url=" + trans , function(json){
                cb('<div class="' + cl1 + '">' + transAdabter(json.tafsir, trans) + '</div>');
            }, 'json');
  ...
  // Get the search query from the search field element
  var query = trim($("#fld_search_txt").val() || $("#fld_search").val());

  // Check if the search query is less than 3 characters in length
  if (query.length < 3) {
  ...
  // Send the HTTP GET request to the search.php file with the search query as a parameter
  $.get("search.php?query=" + query, function(response) {
    ...
    // Construct an HTML string containing the results
  ...
  nass +=
    '<div class="' +
    cls +
    '"> <a href="javascript:;" id="res__' +
    results[i].sura +
    '_' +
    results[i].aya +
    ' " >' +
    parseAr(QuranData.Sura[results[i].sura][sura_key]) +
    " - " +
    parseAr(_lang["aya"]) +
    " " +
    parseAr(results[i].aya) +
    "</a>" +
    results[i].text +
    "</div>";
}
    ...
    // Set up a click handler for the search results links
    $("#search_res a").on("click", function(e) {
      ...
      search_gotoAya(
        $(this).attr("id").split("__")[1].split("_")[0],
        $(this).attr("id").split("__")[1].split("_")[1]
      );
    });
  });
}
...
function showCopyWidg(){
  ...
  $.getJSON('sura-aya.php', { sura: sura, aya: aya }, function(json) {
  // Check if the server returned an error or undefined data
  if (json == undefined || json.error) {
    // Display an error message if there was a problem
    SG.Dialog.alert(_lang["err"]);
    return;
  }
  // Display the verse's text in the dialog
  var nass = '"' + json.text + '" (' + QuranData.Sura[sura][4] + ' : ' + aya + ')';
